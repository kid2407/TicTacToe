...
class World {
	...
	public function isWon(): bool {
		...
		// horizontal
		for ( $x = 0; $x < 2; $x ++ ) {
			if ( $playground[ $x ][0] !== " " && count( array_unique( $playground[ $x ] ) ) === 1 ) {
				return true;
			}
		}
		...
		foreach ( $playground as $key => $row ) {
			if ( $row[0] !== ' ' && count( array_unique( [ $playground[0][ $key ], $playground[1][ $key ], $playground[2][ $key ] ] ) ) === 1 ) {
				return true;
			}
		}

		// diagonals
		if ( $playground[1][1] !== " " && ( count( array_unique( [ $playground[0][0], $playground[1][1], $playground[2][2] ] ) ) === 1 || count( array_unique( [ $playground[2][0], $playground[1][1], $playground[0][2] ] ) ) === 1 ) ) {
			return true;
		}
	...
	public function isDraw(): bool {
		...
		// horizontal
		for ( $x = 0; $x < 2; $x ++ ) {
			if ( $this->isNotDrawForRow( array_unique( $playground[ $x ] ) ) ) {
				return false;
			}
		}
		...
		// vertical
		foreach ( $playground as $key => $row ) {
			if ( $this->isNotDrawForRow( array_unique( [ $playground[0][ $key ], $playground[1][ $key ], $playground[2][ $key ] ] ) ) ) {
				return false;
			}
		}
		...
		if ( $this->isNotDrawForRow( array_unique( [ $playground[0][0], $playground[1][1], $playground[2][2] ] ) ) ) {
			return false;
		}

		//bottom-left -> top-right
		if ( $this->isNotDrawForRow( array_unique( [ $playground[2][0], $playground[1][1], $playground[0][2] ] ) ) ) {
			return false;
		}
	 ...
	 * @return bool
	 */
	private function isNotDrawForRow( array $uniqueValues ) {
